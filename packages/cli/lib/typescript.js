...
function generate(contract, bytecode) {
    ...
    lines.push("    }");
    lines.push("");
    lines.push("    connect(providerOrSigner: ethers.Signer | ethers.providers.Provider): ".concat(contract.name, " {"));
    ...
    contract.interface.fragments.forEach(function (fragment) {
        lines.push("            \"".concat(fragment.format(ethers_1.ethers.utils.FormatTypes.full), "\","));
    });
    lines.push("        ];");
    lines.push("    }");
    lines.push("}");
